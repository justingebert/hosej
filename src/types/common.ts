...
export type AsJson<T> = T extends Document // Only apply `_id` for Mongoose documents
    ? { _id: string } & {
          [K in keyof WithoutMongoose<T> as WithoutMongoose<T>[K] extends Function
              ? never
              : K]: WithoutMongoose<T>[K] extends Types.ObjectId
    ...
    : {
          [K in keyof T as T[K] extends Function ? never : K]: T[K] extends Types.ObjectId
              ? string
              : T[K] extends Date
                ? string
