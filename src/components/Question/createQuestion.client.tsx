...
const CreateQuestion = ({ questionData, setQuestionData }: CreateQuestionProps) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (questionData.questionType === "image-select-one" || questionData.questionType === "image-select-multiple") {
  ...
  return (
    <>
      ...
      <div ref={buttonRef} className="flex justify-center fixed bottom-6 left-0 w-full p-6 bg-background">
        <Button onClick={handleSubmit} disabled={uploading || isSubmitting || !questionData.question.trim() || !questionData.questionType} className="h-12 w-full">
