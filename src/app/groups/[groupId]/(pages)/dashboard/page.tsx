...
import { BarChartBig, ScanSearch, MousePointerClick, CircleSlash, Info, Clock, Users } from "lucide-react";
...
export default function Dashboard() {
    ...
    const { data: globalFeatures, isLoading: featuresLoading } = useSWR<{
                ...
                <div className="flex flex-col gap-4 max-w-2xl mx-auto w-full">
                    ...
                    {!groupLoading || group ? (
                        <>
                            ...
                            ) : globalFeatures?.jukebox?.status === 'enabled' && group?.features?.jukebox?.enabled && (
                                <div
                                    className="relative bg-primary-foreground px-6 py-4 flex items-center justify-between rounded-lg cursor-pointer hover:bg-primary-foreground/80 transition-colors"
                                    onClick={() => {
                                        triggerHaptic();
