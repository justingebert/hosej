    ...
    async (
        ...
        if (rallies.length === 0) {
            return NextResponse.json(
                { message: "No active rallies", rallies: [] },
                { status: 200 }
            );
        }
        ...
        for (const rally of rallies) {
            ...
            // end rally and active new ones
                ...
                const gapEndTime = new Date(
                    currentTime.getTime() +
                        group.features.rallies.settings.rallyGapDays * 24 * 60 * 60 * 1000
                );
                ...
                if (newRally) {
                    ...
                    await newRally.save();
                    await sendNotification(
                        `📷 ${group.name} Rally finished! 📷`,
                        `📷 Next Rally starting: ${newRally.startTime.toLocaleString()}📷`,
                        group._id
                    );
                } else {
                    return NextResponse.json(
                        { message: "No rallies left", rallies: [] },
                        { status: 200 }
                    );
                }
            }
