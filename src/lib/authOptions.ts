...
export const authOptions =
    {
        ...
        callbacks: {
            ...
            async session({session, token}: any) {
                ...
                const user = await User.findById(token.userId).select('-googleId -deviceId');

                session.user = user; // Include the full user object in the session -- this is bad , needs to be fixed
                return session;
            },
        },
        pages: {
